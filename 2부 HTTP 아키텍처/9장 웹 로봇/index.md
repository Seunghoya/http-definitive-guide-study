...
- 로봇은 From, User-Agent 등의 헤더를 통해 신원 정보를 넘기고 사이트 관리자가 로봇에 대해 문의나 불만사항이 있을 경우를 위해 연락처를 제공해야 한다.
  ```http
...
- 로봇 이름을 대소문자를 구분하지 않는 부분 문자열로 대응하므로 의도치 않게 맞는 경우에 주의해야 한다.
  (ex. 'User-Agent: bot'은 Bot, Robot, Bottom-Feeder, Spambot 등에 매칭됨)
